...
void RESERVAR_POLTRONA(SESSAO *RESERVAR, int FILA, int CORREDOR){
	if(RESERVAR->UMA_SESSAO[FILA][CORREDOR] == 0){
		...
		cout << "NÃƒO foi possÃ­vel reservar a poltrona[" << FILA << "][" << CORREDOR << "]!!!" << endl;
		cout << "Escolha a opÃ§Ã£o novamente para outra poltrona" << endl;
		system("pause");		
	}
}
...
int VERIFICA_ENTRADA_CORREDOR(int CORREDOR){
	if((CORREDOR<0) or (CORREDOR>21) or (CORREDOR == 10)){
		cout << "Os corredores devem estar entre 0 e 20, exluÃ­do o corredor 10 que nÃ£o possui assentos!" << endl << endl;
		return(0);
	}else{
		return(1);
	}	
